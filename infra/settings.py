...
from infra.paths import user_data_dir, resource_path, libs_dir

SETTINGS_FILE: Path = user_data_dir() / "ssaa_settings.json"

def _defaults() -> Dict[str, Any]:
    # Base packaged libs (inside resources/)
    base_consumos = resource_path("consumos.lib")
    # Historical baseline file name
    base_materiales = resource_path("materiales_updated_2.lib")
...
def ensure_seed_libs() -> Dict[str, Any]:
    ...
    if base_consumos.exists() and not user_consumos.exists():
        shutil.copy2(base_consumos, user_consumos)

    if base_materiales.exists() and not user_materiales.exists():
    ...
    if not cons_p.exists() and user_consumos.exists():
        s["consumos_lib_path"] = str(user_consumos)

    if not mat_p.exists() and user_materiales.exists():

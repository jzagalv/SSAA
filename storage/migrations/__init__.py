...
def upgrade_project_dict(data: Any, *, to_version: int) -> Dict[str, Any]:
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).debug("Failed to set schema_version (best-effort).", exc_info=True)
    ...
    try:
        ...
        if not isinstance(proy.get("cc_escenarios"), dict):
            proy["cc_escenarios"] = {}


        # cc.scenarios (legacy contract) -> cc_escenarios
        cc = proy.get("cc")
        if isinstance(cc, dict):
            cc_scen = cc.get("scenarios")
            if isinstance(cc_scen, dict):
                for k, v in cc_scen.items():
                    key = str(k)
                    if key not in proy["cc_escenarios"] or not str(proy["cc_escenarios"].get(key) or "").strip():
                        proy["cc_escenarios"][key] = str(v) if v is not None else ""
            cc_other = {k: v for k, v in cc.items() if k != "scenarios"}
            if not cc_other:
                proy.pop("cc", None)
            else:
                proy["cc"] = cc_other
    ...
    # ---- Compat: cc_scenarios_summary dict -> calculated.cc.scenarios_totals ----
    try:
        proy = d.get("proyecto", {})
        if isinstance(proy, dict):
            legacy = proy.get("cc_scenarios_summary")
            if isinstance(legacy, dict):
                calc = proy.get("calculated")
                if not isinstance(calc, dict):
                    calc = {}
                    proy["calculated"] = calc
                cc_calc = calc.get("cc")
                if not isinstance(cc_calc, dict):
                    cc_calc = {}
                    calc["cc"] = cc_calc
                if cc_calc.get("scenarios_totals") != legacy:
                    cc_calc["scenarios_totals"] = legacy
                proy["cc_scenarios_summary"] = []
            elif not isinstance(legacy, list) and legacy is not None:
                proy["cc_scenarios_summary"] = []
    except Exception:
        import logging
        logging.getLogger(__name__).debug("Failed to migrate cc_scenarios_summary dict (best-effort).", exc_info=True)
